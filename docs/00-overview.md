...
> **边界一分为二**（2026-08-16）：
> - **对内**：编排者 / 执行者 / 看护者 / 工具门禁构成一个有纪律的星座——内部接缝禁 agent 直连，同步走共享事实层（见 [01 拓扑](01-topology.md)）。
> - **对外**：星座作为整体可暴露**对外互操作边界**，与外部智能体/智能体团体对等协作（A2A 任务对话 + ANP 身份信任，子域式统一入口 + overlay 多级回落，见 [12 对外互操作](12-external-interop.md)）。对外只暴露统一入口，内部组织不对外泄露。
