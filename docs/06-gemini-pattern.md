...
### 5.1 模型档位（tier-ref）说明

`model` 字段是**引用名**，不是模型本身。它指向模型配置层（L2）——主模型 + 回落链定义在那里，注册表不复制具体模型名，避免一处修改、多处失同步（引用协议 `tier-ref`）。

档位是**能力档位**，不是具体模型：

| 档位 | 语义 | 适用场景 |
|:-----|:-----|:---------|
| `standard` | 全能力档：主模型 + 完整回落链 | 长编码、跨机构建-修复循环、验收复核等需要完整推理的任务 |
| `light` | 轻量档：成本/速度优先，简化回落链 | 日常运维、状态检查、单文件小改、并行子任务等常规任务 |

档位到具体模型的映射（哪个模型叫 standard、哪个叫 light）由部署者在模型配置层（L2）决定——注册表只声明「这个智能体需要什么能力档位」，不关心背后是哪个模型。这保持了注册表的通用性：换模型只改 L2，注册表一行不动。
