...
> 0.1.0 已发布（gitea 私有，v0.1.0，2026-08-04）；**v0.2.0（2026-08-14，ACP 化 + dsh 执行者版）**；**v0.2.1（2026-08-15，添加 dsh 作为推荐执行者之一 + A2A 支持核实落地）**；**v0.2.2（2026-08-15，总是知晓保证机制——编排者/看护者架构感知常驻化）**；**v0.2.3（2026-08-16，对外互操作门面——A2A+ANP 分层、泛域子域式暴露、内部 overlay 多级回落）**；**v0.2.4（2026-08-16，对外身份层落地 did:wba——域名身份 + HTTPS 公开 DID 解析 + 原生 verifier，编排者/看护者 skill 与教程同步 did:wba 衔接）**；**v0.2.5（2026-08-16，对端互连操作手册——docs/12 §12.10 复制即用的 A2A 卡片/消息与 ANP 签名客户端 + 故障排查表）**；**v0.2.6（2026-08-16，外部任务派发桥——对外 A2A/ANP 端点验签后把带调用身份的任务转给本机 Hermes 执行，docs/12 §12.11 did:wba 迁移清单 + §12.12 派发桥，HC01/NUC10 双机按规范部署）**；**v0.2.7（2026-08-19，内部多会话协作 + ANP phase1 退休——docs/13 定义队内多个 Hermes 会话经 A2A 的分身/知会/协作/记忆模型与能力边界，默认 A+B 共享真相源、a2a_call 默认不开放、同 context 再进入保有会话级记忆实测；同时**退休 ANP phase1（本地预共享 DID 目录）过渡形态**，信任层统一为真 did:wba——删 templates/external-interop/anp_auth.py、`anp_server_main.py` 去 AUTH_MODE 分支只留 didwba、docs/12 §12.11 改写为「启用真 did:wba」清单）**；正式版 v1.0.0 待 AIGate 开发完成 + 实际部署验证后再出。
...
- [x] **对外身份层落地 did:wba（2026-08-16，v0.2.4）**— ANP 身份/信任层从「本地预共享 DID 文档」升级为 **did:wba 域名身份 + HTTPS 公开 DID 解析 + 原生 verifier**：身份 `did:wba:<对外子域>`；DID 文档经 `/.well-known/did.json` + `/{agent}/did.json` 公网 HTTPS 可解析；OpenANP `DidWbaVerifier` 对端做真实网络解析。身份 key 用 **k1 profile（secp256k1/JWK）**、JWT 用 **ES256（EC P-256）**（两项都是验证过的避坑选择）。编排者/看护者 skill 与教程（docs/12 §12.9）已同步 did:wba 衔接。启用 did:wba 后，`did:all` 旧身份对端须先迁移为 did:wba（原生 verifier 只认 did:wba）。完整闭环已实测：签名 → HTTPS 门面 → resolve DID → 验签 → 响应 200。
- [x] **内部多会话协作（2026-08-19，v0.2.7）**— docs/13-multi-session-collaboration.md 定义队内多个 Hermes 会话经 A2A 的分身/知会/协作/记忆模型与能力边界：① **分身 vs 真多智能体**（同一 profile 多会话 = 分身；多个独立 profile = 真多智能体，后者才隔离 memory/视角）先钉死；② 三种用法从弱到强（A+B 共享真相源 + A2A 知会为 MUST 默认、请求协作实际改码、纯 B 退化为编排者搬运——被调用会话 toolset 常无 a2a 只能收不能发）；③ **记忆模型**（全局 memory 不随 A2A 自动同步；会话级记忆存于持久化 context 档案，同 context 再进入保有协作记忆——已实测）；④ **安全**：`a2a_call` 默认不开放，开放 MUST 配对等体白名单。编排者 skill 与教程同步内部多会话衔接。
- [x] **ANP phase1 过渡形态退休（2026-08-19，v0.2.7）**— 既然已实现完全规范的 ANP DID（did:wba），phase1（本地预共享 DID 目录 + http_signature 中间件）作为过渡形态不再需要：删除 `templates/external-interop/anp_auth.py`（phase1 专属模块）、`anp_server_main.py` 去掉 `ANP_AUTH_MODE` 开关/`trusted-dids`/did:all 回退分支只剩 didwba、README 同步；docs/12 §12.11 改写为「启用真 did:wba（操作清单）」（保留迁移/陷阱要点，删 phase1 叙事）、§12.9 演进措辞清理；skill 同步「ANP trust = did:wba only」。templates 目录仅剩 `a2a_server_main.py` / `anp_server_main.py` / `dispatch.py` / `README.md`。
