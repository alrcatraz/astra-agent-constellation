...
- **External interop (A2A/ANP + did:wba)** — the public-facing boundary is a
  *separate* concern from the internal Hermes A2A ladder above. When a task
  ...
  For upgrading a member ANP from *phase1 (pre-shared trust)* to real did:wba
  (k1 identity + native network DID resolution), see
  `docs/12-external-interop.md` §12.11 (verified operation checklist). For the
  dispatch bridge — outward A2A/ANP endpoints handing validated external tasks
  (with caller identity) to the *local* Hermes agent via `hermes -z` — see
  §12.12. The templates live in `templates/external-interop/`.
